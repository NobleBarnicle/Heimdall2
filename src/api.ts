...
export type Document = { id: string; title: string; neutral_citation: string | null; court: string | null; decision_date: string | null; source_filename: string; extraction_status: string; created_at: string };
export type Paragraph = { id: string; ordinal: number; label: string; text: string; source_page_start: number; source_page_end: number };
export type Annotation = { id: string; document_id: string; paragraph_ids: string[]; proposition: string; decision_track: string | null; bail_proceeding: string | null; bail_issue: string | null; bail_result: string | null; bail_factors: string[]; annotation_type: string; areas: string[]; authority_weight: string; function: string; relationship_type: string | null; boundary: string | null; triggers: string[]; commentary: string | null; related_authorities: string[]; ontology_version: string; created_at: string; updated_at: string };
...
export const api = {
  ...
  annotations: (query = "") => request<Annotation[]>(`/annotations${query ? `?q=${encodeURIComponent(query)}` : ""}`),
